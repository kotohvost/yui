...
$Id: regexpr.c,v 3.2.2.1 2007/07/24 09:58:07 shelton Exp $
...
static int regexp_syntax = RE_SYNTAX_AWK | RE_ANSI_HEX ;
...
regexp_t bufp;
{
  ...
  if (bufp->used <= sizeof(small_visited))
    visited = small_visited;
  else
    {
      visited = malloc(bufp->used);
      if (!visited)
       return 0;
    }


  for (pos = 0; pos < bufp->used;)
	 {
	   memset(visited, 0, bufp->used);
	   re_compile_fastmap_aux(bufp->buffer, pos, (char *)visited, &can_be_null, map);

	   switch (bufp->buffer[pos++])
	     {
	     case Cend:
		 break;
	     case Cbol:
...
static re_pattern_buffer re_comp_buf;
...
const char *s;
{
  ...
  return re_compile_pattern((char *)s, strlen(s), &re_comp_buf);
}
...
const char *s;
{
  ...
  return re_search(&re_comp_buf, (char *)s, len, 0, len, (regexp_registers_t)NULL) >= 0;
}
...
int main()
{
  ...
  while (1)
	...
	for (pos = 0; pos < exp.used;)
	 {
	   char cbn;
	   //re_calc_map(exp.buffer, exp.used, pos, &cbn, map);
	   printf("%d: ", pos);
