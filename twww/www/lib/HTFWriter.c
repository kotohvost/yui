...
char * cache_filename( char *dir, char *url, const char *suffix )
{
  static char urlBuf[256];
  char *buf=(char *)malloc(256);
  char *ret=buf, *p, *Url=urlBuf;
  int done=0, le;
...
static FILE * create_file(char *Name)
{
  ...
  for(i=0; i<n-1; i++)
   { /* try cd to dir and create dir*/
     if (chdir(vect[i])==-1)
      {
	if ( mkdir(vect[i], 0777)==-1 )
	   return 0;
	if ( chdir(vect[i])==-1 )
	   return 0;
      }
...

{
    ...
    fnam = HTFWriter_filename( HTCacheDir,
			      ...
			      0);
    */
    url = HTAnchor_address((HTAnchor*)request->anchor);
    if ( strstr(url, "/cgi-bin/") || strchr(url, '?') )
     {
       free(me);
       return 0;
     }
    fnam = cache_filename( HTCacheDir, url, HTFileSuffix(input_format) );

    me->filename = NULL;
		 /* Limit number (not size) of files */
    /*
    limit_cache(HTCache);
    */
    /*
    me->fp = fopen (fnam, "w");
    */
    me->fp = create_file( fnam );
    if (!me->fp) {
	static char str[256];
	sprintf(str, "Can't open local file to write cash. (%s)", fnam);
	HTAlert(str);
	free(fnam);
	free(me);
	return NULL;
