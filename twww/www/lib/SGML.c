...
#endif
{
	...
	le=strlen(entities[i]);
	diff=strncasecmp(entities[i], s, le);
...
normal_text:
    ...
    case S_nl:
	if (c=='<') {
	    PUTC(' '); /* Для вызова flush, чтобы не терялось последнее слово */
	    string->size = 0;
	    context->state = (context->element_stack &&
		context->element_stack->tag  &&
		context->element_stack->tag->contents == SGML_LITERAL) ?
...
handle_S_tag:
	...
	if (WHITE_CHAR(c)) break;	/* Gap between attributes */
	if (c=='>') {		/* End of tag */
	    if (context->current_tag->name) start_element(context);
	    context->state = S_after_open;
	    break;
	...
	break;
				...
				/* accumulating value */
    case S_attr:
	if (WHITE_CHAR(c) || (c=='>') || (c=='=')) {		/* End of word */
	    HTChunkTerminate(string) ;
	...
	if (WHITE_CHAR(c)) break;	/* Gap after attribute */
	if (c=='>') {		/* End of tag */
	    if (context->current_tag->name) start_element(context);
	    context->state = S_after_open;
	    break;
	...
	if (WHITE_CHAR(c)) break;	/* Before attribute value */
	if (c=='>') {		/* End of tag */
