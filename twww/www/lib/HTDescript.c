...
PUBLIC HTList * HTReadDescriptions ARGS1(char *, dirname)
{
	...
	while (*s && WHITE_CHAR(*s)) s++;		/* Skip initial whitespace */
	if (*s!='d' && *s!='D') continue;	/* Junk non-description lines*/

	t = s+1;
	while (*t && !WHITE_CHAR(*t)) t++;	/* Find the end of the keyword */
	while (*t && WHITE_CHAR(*t)) t++;	/* Find the beginning of template */

	if (*t) {
	    d = t+1;
	    while (*d && !WHITE_CHAR(*d)) d++;	/* Find end of template */
	    if (*d) {
		*d++ = 0;			/* Terminate template */
		while (*d && WHITE_CHAR(*d)) d++;	/* Find start of description */
		if (*d) {
		    char * p = d;
		    while (*p && *p!='\r' && *p!='\n') p++;
