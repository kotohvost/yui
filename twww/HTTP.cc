...
extern char * HTHostName;	       /* имя хоста для виртуальных хостов */
	...
	if (strchr(line, ':')) {	/* Valid header line */
	    ...
	    if (0==strcasecomp(fieldname, "Proxy-Authenticate:")) {
		  ...
		  free( req->proxy_authorization );
	       req->proxy_authorization = strdup( "required" );
	       */
	       goto m1;
	    }
				  ...
				  http_info *, http, char *, url)
{
	...
	if (request->parentAnchor) {
	    char *_this = HTAnchor_address((HTAnchor *) request->anchor);
	    char *parent = HTAnchor_address((HTAnchor *)request->parentAnchor);
	    char *relative = HTParse(parent, _this,
	...
	if ( HTHostName != NULL ) {
	    sprintf(line, "Host: %s%c%c", HTHostName, CR, LF);
	    HTChunkPuts(command, line);
	}
	...
	/* Put out proxy-authorization */
	if (request->proxy_authorization != NULL) {
	    HTChunkPuts(command, "Proxy-Authorization: ");
	    HTChunkPuts(command, request->proxy_authorization);
	    HTChunkPutc(command, CR);
	    HTChunkPutc(command, LF);
	}

	W3Win *w = (W3Win*)(((winExecutor*)request->context)->win);
	if ( w->submit_data )
	  {
	    sprintf( line, "Content-type: %s%c%c", "application/x-www-form-urlencoded", CR, LF );
	    HTChunkPuts( command, line);
	    sprintf( line, "Content-length: %d%c%c%c%c", strlen(w->submit_data), CR, LF, CR, LF);
	    HTChunkPuts( command, line );
	    HTChunkPuts( command, w->submit_data );
	  }
			      ...
			      HTFormat, format_in, BOOL, use_cache)
{
	...
	HTCopy(http->sockfd, target, request);		     /* USE RETURN AS STATUS */
	FREE_TARGET;
	status = HT_LOADED;
    }
    return status;
...
*/
extern "C" PUBLIC int HTLoadHTTP ARGS1 (HTRequest *, request)
{
    char *url;
...
*/
	...
	*/
	HTCopy(http->sockfd, request->output_stream, request);
...
*/
	...
	if (!status_line) {
	    ...
	    */
		format_in = WWW_HTML;
	    if ( request->realm )
	       request->realm->success = YES;
	    if ( request->proxy_realm )
	       request->proxy_realm->success = YES;
	    ...
	    */
		...
		ret = sscanf(status_line, formatstr, server_version, &server_status);
		if ( ret < 2 ) {
		...
		*(server_version+VERSION_LENGTH) = '\0';
		...
		switch (server_status) {
		  ...
		  case 407:
		    ...
		    http->sockfd = -1;
		    if (HTAA_retryWithAuth(request, HTLoadHTTP, server_status==407?YES:NO)) {
