...
int w3system( const char *cmd, HTRequest *request );

#define system( cmd )	w3system( cmd, request );
...
**	-------------------------
*/
PRIVATE int remote_session ARGS3(HTRequest *, request, char *, access, char *, host)
{
	char * user = host;
...
PRIVATE int HTLoadTelnet ARGS1(HTRequest *, request)
{
    ...
    status = remote_session(request, access, host);
